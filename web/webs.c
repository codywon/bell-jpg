...
static void websSocketEvent( int sid, int mask, int iwp )
{
    ...
    {
        printf( "websReadEvent\n" );
        websReadEvent( wp );
        printf( "websReadEvent1\n" );
    }

    if ( mask & SOCKET_WRITABLE )
    {
        //printf("websReadEvent2\n");
        if ( websValid( wp ) && wp->writeSocket )
        {
            ( *wp->writeSocket )( wp );
        }

        //printf("websReadEvent3\n");
    }
}
    ...
    for ( done = 0; !done; )
    {
        ...
        switch ( wp->state )
        {
            case WEBS_BEGIN:
                ...
                //printf("websParseFirst1\n");
                wp->state = WEBS_HEADER;
                //printf("websParseFirst2\n");
                break;
...
static int websGetInput( webs_t wp, char_t** ptext, int* pnbytes )
{
    ...
    *pnbytes = 0;
    ...
    if ( len > 0 )
    {
        ...
        if ( nbytes < 0 )  						/* Error */
        {
            //printf("websGetInput3\n");
            websDone( wp, 0 );
            //printf("websGetInput4\n");
            return -1;
        }
    ...
    else
    {
...
#else
        //printf("websGetInput7\n");
        nbytes = socketGets( wp->sid, &text );
        //printf("websGetInput8\n");
#endif
        ...
        if ( nbytes < 0 )
        {
...
#else
            //printf("websGetInput9\n");
            eof = socketEof( wp->sid );
            //printf("websGetInputa\n");
#endif
            ...
            if ( eof )
            {
                ...
                {
                    printf( "websGetInputc\n" );
                    websUrlHandlerRequest( wp );
                    printf( "websGetInputd\n" );
                }
            ...
            {
                //printf("websGetInput10\n");
                websParseRequest( wp );
                websUrlHandlerRequest( wp );
                //printf("websGetInput11\n");
            }
        ...
        else if ( nbytes == 0 )
        {
            if ( wp->state == WEBS_HEADER )
            {
                 ...
                 */
                //printf("websGetInput12\n");
                websUrlHandlerRequest( wp );
                //printf("websGetInput13\n");
            }
