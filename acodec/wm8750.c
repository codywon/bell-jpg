...
extern unsigned char i2c_WM8751_write( unsigned short sub_addr, unsigned short value );
...
void wmcodec_write(int reg_addr, unsigned long reg_data)
{
	...
	Textout("[WM875X(%02X)=0x%08X]",(unsigned int)reg_addr,(unsigned int)reg_data);
...
{
	//Textout("audiohw_set_lineout_vol");
	switch(Aout)
	{
	case 1:
...
{
	//Textout("audiohw_set_linein_vol");
...
{
	//Textout("audiohw_set_frequency=0x%08x",fsel);
